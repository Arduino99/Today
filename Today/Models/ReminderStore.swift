...
//  Today
//
//  Created by Arduino Mandarelli on 12/03/2023.
//
    ...
    func requestAccess() async throws {
        ...
        case .notDetermined:
            ...
            if #available(iOS 17.0, *) {
                accessGranted = try await ekStore.requestFullAccessToEvents()
            } else {
                accessGranted = try await withCheckedThrowingContinuation { continuation in
                    ekStore.requestAccess(to: .reminder) { (granted: Bool, error: Error?) in
                        if let error = error {
